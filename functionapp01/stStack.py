...
for s in storageAccounts:
    ...
    key = storage.list_storage_account_keys_output(
        account_name=st.name,
        resource_group_name=s.get("rgName", resource_group.name),
    )

    connection_string = key.apply(
        lambda args: f"DefaultEndpointsProtocol=https;AccountName={s["name"]};AccountKey={args.keys[0].value};EndpointSuffix=core.windows.net"
    )
    st_key_dict[s["name"]] = connection_string
    st_dict[s["name"]] = st

    blob = storage.BlobContainer(
        resource_name=f"func-{s["name"]}",
        container_name="func",
        account_name=st.name,
        resource_group_name=s.get("rgName", resource_group.name),
    )
